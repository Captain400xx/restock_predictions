# ----------------------------------------------------------------------
# Pokémon Card Drop Forecast - v6.7 (Final Bug Fixes)
# ----------------------------------------------------------------------
...
import numpy as np
import os
import streamlit.components.v1 as components # For the timer
...
def train_ml_model(data, forecast_horizon, model_type='lgbm'):
    ...
    if model_type == 'lgbm': model = lgb.LGBMRegressor(random_state=42)
    elif model_type == 'xgb': model = xgb.XGBRegressor(random_state=42)
    else: model = cb.CatBoostRegressor(random_state=42, verbose=0)
...
# --- CHANGE 3: Rewritten Chart Function ---
def create_forecast_chart(forecast_df, big_restocks_df, retailer, title):
    ...
    if not big_restocks_df.empty:
        fig.add_trace(go.Scatter(
            x=big_restocks_df["ds"], y=big_restocks_df["yhat"], 
            mode="markers", name="Predicted Big Restock", 
    ...
    if not forecast_df.empty:
        ymax = max(forecast_df['yhat'].max() * 1.2, 5) 
    else:
        ymax = 5

    # Then, update the layout with both axes
    fig.update_layout(
        ...
        height=500,
        yaxis=dict(
            title="Predicted Restock Activity",
            range=[0, ymax]
        ),
        # BOTTOM X-AXIS (Date)
        xaxis=dict(
            title="Date (US/Eastern)",
            tickformat="%a %m/%d",
            dtick="D1",
            showgrid=True,
        ...
        # TOP X-AXIS (Time)
        xaxis2=dict(
            title="Time of Day (ET)",
            overlaying='x',  # Link to the bottom x-axis
            side='top',
            tickformat='%I %p',  # e.g., "12 PM", "04 PM"
            dtick=3600000 * 4,  # Ticks every 4 hours
            showgrid=True,
            gridcolor='rgba(255, 0, 0, 0.3)', # Red gridlines
            gridwidth=1,
            showticklabels=True
        ),
...
# --- CHANGE 2: Rewritten Schedule Function ---
def display_consensus_schedule(df):
    st.markdown("""
    <style>
        .schedule-table { width: 100%; border-collapse: collapse; font-size: 0.9em; }
        .schedule-table th, .schedule-table td { padding: 8px; text-align: left; border-bottom: 1px solid #444; }
        .schedule-table th { background-color: #1a1a1a; }
        .high-confidence { background-color: rgba(40, 167, 69, 0.3); }
        .medium-confidence { background-color: rgba(255, 193, 7, 0.3); }
        .low-confidence { background-color: rgba(220, 53, 69, 0.3); }
        .day-group { font-weight: bold; font-size: 1.1em; padding-top: 15px; }
    </style>
    ...
    if df.empty:
        ...
        # Sort the DataFrame by time_group to ensure chronological order
        ...
        for _, row in df_sorted.iterrows():
            ...
            # If this row is a new day, print the day header
...
def create_importance_chart(df):
    ...
    fig.update_layout(
        title="Model Feature Importance (All Models)",
...
# --- CHANGE 1: Restore "How to Use" text ---
...
if not retailers: st.error("No retailers found in the data. Please check your CSV file."); st.stop()
...
countdown_placeholder = st.sidebar.empty() # Timer placeholder

st.sidebar.markdown("---")
if os.path.exists("logo.png"):
    st.sidebar.image("logo.png", width=100)
st.sidebar.warning("This data is the property of Emerald Alerts. Unauthorized sharing, distribution, or external use of this information may result in penalties or legal action.")
st.sidebar.markdown("---")
...
if not consensus_summary.empty and "High" in consensus_summary['Confidence'].values:
    ...
    # --- CHANGE 1: Use components.html for robust timer ---
    now_eastern = datetime.now(EASTERN)
    ...
    js_countdown = f"""
    <h2 id="countdown" style="text-align: left; font-weight: bold; color: #28a745;"></h2>
    <script>
    ...
    st.info("No 'High Confidence' restocks (where 3+ models agree) were found in the forecast horizon.")
    countdown_placeholder.info("No high-confidence alert scheduled.")

# -------------------------
# 7. Main Tabs
# -------------------------
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["⭐ Prediction Schedule", "📊 Analysis", "🔮 Prophet", "🚀 LightGBM", "🌟 XGBoost", "🐾 CatBoost"])
with tab1:
    st.header("Prediction Schedule")
    st.markdown("This schedule shows predicted 'big restock' time windows, colored by confidence level.")
...
with tab2:
    ...
    st.markdown("This calendar shows which days of the month are predicted to have the most activity.")
    ...
    st.markdown("This chart shows which hours of the day are predicted to have the most activity.")
    if not prophet_forecast.empty:
        hourly_fig = create_hourly_heatmap(prophet_forecast, retailer)
        st.plotly_chart(hourly_fig, use_container_width=True)
        
    st.subheader("Model Feature Importance")
    st.markdown("This chart shows which factors are the most important for making predictions, averaged across all ML models. A higher value means the factor is more influential.")
    fig_imp = create_importance_chart(importance_df)
    st.plotly_chart(fig_imp, use_container_width=True)

with tab3:
    st.header("🔮 Prophet Model Details")
    fig_prophet = create_forecast_chart(prophet_forecast, prophet_big, retailer, "Prophet Forecast")
    st.plotly_chart(fig_prophet, use_container_width=True)
with tab4:
    st.header("🚀 LightGBM Model Details")
    fig_lgbm = create_forecast_chart(lgbm_forecast, lgbm_big, retailer, "LightGBM Forecast")
    st.plotly_chart(fig_lgbm, use_container_width=True)
with tab5:
    st.header("🌟 XGBoost Model Details")
    fig_xgb = create_forecast_chart(xgb_forecast, xgb_big, retailer, "XGBoost Forecast")
    st.plotly_chart(fig_xgb, use_container_width=True)
with tab6:
    st.header("🐾 CatBoost Model Details")
    fig_cat = create_forecast_chart(cat_forecast, cat_big, retailer, "CatBoost Forecast")
